...
export default function Cart({ allProduct }) {
  const { productName, price, rating, status, image, id } = allProduct;
  return (
    <>
      <div class="relative m-10 flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
        ...
        <div class="mt-4 px-5 pb-5">
          <Link href="/product/1">
            <h5 class="text-xl tracking-tight text-slate-900">{productName}</h5>
          </Link>

          <p>
            <span class="text-lg font-bold py-9 text-slate-900">
              Category: {status}
            </span>
          </p>
